...
class SIM800L:
    def __init__(self,ser):
        try:
            self.ser=serial.Serial("/dev/serial0", baudrate=9600, timeout=5)
    ...
    def check_incoming(self):
        if self.ser.in_waiting:
            ...
            if params[0][0:5] == "+CMTI":
                ...
                #@todo handle
                pass
    def gprs_gsm(self,length,data):
        #s='GET /~mayagree/api.php?MSG=CID:1;S1:050_S2:080_S3:080;V1:0ON_V2:OFF_V3:0ON;M1:OFF;\r\n'
        self.command('AT+QIOPEN="TCP","103.235.106.84","80"\n')
        time.sleep(2)
        self.command('AT+QISEND=%s\r\n'%length)
        time.sleep(2)
        self.command(data)
        time.sleep(2)
        self.command('AT+QICLOSE')
        time.sleep(2)
